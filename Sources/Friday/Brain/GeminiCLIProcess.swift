...
// Coding agent backend that delegates to the Gemini CLI.
...
final class GeminiCLIProcess: @unchecked Sendable, CodingAgentProcess {

    let agentName = "Gemini CLI"
    private(set) var isBusy = false

    private static let notesDirectory = "/Users/papa/Documents/notes"

    /// Resolved once at startup. Checks the Homebrew path first, falls back to `which gemini`.
    private static let geminiPath: String = {
        let homebrew = "/opt/homebrew/bin/gemini"
        if FileManager.default.fileExists(atPath: homebrew) { return homebrew }
        let p = Process()
        p.executableURL = URL(fileURLWithPath: "/bin/zsh")
        p.arguments = ["-lc", "which gemini"]
        let pipe = Pipe()
        ...
        return path.isEmpty ? "gemini" : path
    }()
    ...
    func ask(_ message: String, directory: String? = nil, maxTurns: Int = 15,
        ...
        guard !isBusy else { return "[TASK_ERROR] Gemini CLI is already busy with another task in this project." }
        isBusy = true
        defer { isBusy = false }

        print("Friday: → gemini \(directory ?? "notes") '\(message.prefix(60))'")
        let response = try await runStreaming(prompt: preamble + message, directory: directory, onProgress: onProgress)
        print("Friday: ← gemini '\(response.prefix(80))'")
        return response
    }
    ...
    private func runStreaming(
        ...
        try await withCheckedThrowingContinuation { cont in
            let p = Process()
            p.executableURL = URL(fileURLWithPath: Self.geminiPath)
            p.arguments = ["--prompt", prompt, "--output-format", "text", "--yolo"]
            ...
            timeoutTimer.setEventHandler {
                if !done && Date().timeIntervalSince(lastOutputTime) > 180 {
                    print("Friday: gemini timed out — no output for 180s")
                    p.terminate()
                }
            }
            timeoutTimer.resume()
            ...
            hardDeadline.setEventHandler {
                if !done {
                    print("Friday: gemini hard timeout — 300s wall clock exceeded")
                    p.terminate()
                }
            }
            hardDeadline.resume()
            ...
            p.terminationHandler = { proc in
                ...
                if let errText = String(data: errData, encoding: .utf8),
                   !errText.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
                    print("Friday: gemini stderr — \(errText.prefix(300))")
                }

                let output = accumulated.trimmingCharacters(in: .whitespacesAndNewlines)
                if proc.terminationStatus == 0, !output.isEmpty {
                    cont.resume(returning: output)
                } else {
                    let reason = output.isEmpty ? "no output" : "exit \(proc.terminationStatus)"
                    cont.resume(returning: "[TASK_ERROR] Gemini CLI failed (\(reason))")
                }
            }
